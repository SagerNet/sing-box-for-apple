...
public class ExtensionEnvironments: ObservableObject {
    ...
    private func resetRemoteSessionState() {
        ...
        private func handleEnterForeground() {
            ...
            guard remoteReconnectPending else { return }
            remoteReconnectPending = false
            if remoteServer != nil, !commandClient.isConnected {
                commandClient.connect()
            }
        }
    #endif
    ...
    private func handleRemoteControlError(_ error: CommandClient.ConnectionError) {
        ...
        if error.kind == .connectionLost {
            ...
            if remoteReconnectAttempts < Self.maxRemoteReconnectAttempts {
                ...
                #if canImport(UIKit)
                    if isInBackground {
                        // Reconnecting while backgrounded would burn the attempt
                        // on a connection iOS is about to suspend; wait for the
                        // next foreground transition instead.
        ...
        let description = remoteSessionHadConnected
            ? "Disconnected from remote server \(server.name)"
            : "Failed to connect to remote server \(server.name)"
        exitRemoteControl()
